...
export class Seaweed {
  ...
  constructor(x: number, ctx: CanvasRenderingContext2D) {
    const { H } = store;
    this.x = x;
    this.baseY = H;
    this.segments = Math.floor(rand(6, 14));
    this.segLen = rand(14, 25);
    this.phase = rand(0, TAU);
    this.speed = rand(0.5, 1.2);
    this.width = rand(4, 10);
